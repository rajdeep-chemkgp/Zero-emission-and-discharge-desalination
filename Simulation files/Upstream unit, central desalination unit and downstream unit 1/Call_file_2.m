...
CO2_emission_per_distillate=CO2_emission/Devaporator
% %% --------------------------------- Solar calculation --------------------------------------------------------
% Qprocess=Qcrystallizer+Qproc;
% [collector_CC, collector_OC,TES_cost, BHX_CC,BHX_OC] = PTSC_Solar_TVC(Qprocess,Mms,Pms,Tvs(1))
% %% --------------------------------- Open raceway pond --------------------------------------------------------
...
Tcc=Capex_direct+Capex_indirect        %Total capital cost ($)
% ------------------------ Csteam calculation -----------------------------
if (Pms<=101.325)
    Csteam_TVC=Csteam;
else 
...
AOC=AOCchem+AOClab+AOCpow+AOCman+AOCsteam_TVC
%----------------------------------- Total Annual cost--------------------
...
FWC=TAC*(Devaporator*THY*3600)^(-1);          %Freshwater cost in $/kg
