...
Msw=Mcw+Ftotal
Dtotal_ZLD=Devaporator+694.251798350868
% Fcrystallizer=Mcw+Brine;
% Xfcrystallizer=(Brine*Xs(n)+Mcw*Xf)/Fcrystallizer;
% Tfcrystallizer=(Tbs(n)*Brine+Mcw*Tf)/Fcrystallizer;

% Fcrystallizer=Brine;
% Xfcrystallizer=Xs(n);
% Tfcrystallizer=Tbs(n);
% 
% C=Crystallizer(Xfcrystallizer,Fcrystallizer,Tfcrystallizer,Ts,Tf,Tcw);
% Msalt=C(1);                                 % Mass flowrate of salt
% Qcrystallizer=C(2);                         % Heat utility of the crystallizer in kJ/s
% Ecrystallizer=C(3);                         % Electrical utility of the crystallizer in kJ/s
% Dcrystallizer=C(4);                         % Distillate from crystallizer
% mhs=C(5);                                   % Flowrate of heat utility
% Mcw_crystallizer=C(6);                      % Mass of cooling water from crystallizer
% Tvcrystallizer=C(7);                        % Temperature of vapor from crystallizer
% Distillate=Devaporator+Dcrystallizer;       % Total distillate from the evaporator and the crystallizer
% LMTDcrystallizer=((Tvcrystallizer-Tf)-(Tvcrystallizer-Tcw))/log((Tvcrystallizer-Tf)/(Tvcrystallizer-Tcw));
% Acrystallizer=C(8);
%% --------------------------------- Converting brine flowrate to m3/day --------------------------------------
% Brine_m3_s=Fcrystallizer/rhol(Tfcrystallizer,Xfcrystallizer*10^6);
% Brine_m3_day=Brine_m3_s*24*3600;
...
%----------------------- MED-PCF-TVC---------------------------------------
Qproc_MED=Mms*Lamda(Tsaturation(Pms))
Qproc_ZLD=374649.232+1275468.45;
Qproc=Qproc_MED+Qproc_ZLD
% Qproc=S*Lamda(Ts);
...
CO2_emission_per_distillate=CO2_emission/Dtotal_ZLD;
%% --------------------------------- Solar calculation --------------------------------------------------------
Qprocess=Qproc
[collector_CC, collector_OC,TES_cost, BHX_CC,BHX_OC,Asolar] = PTSC_Solar_TVC(Qprocess,Mms,Pms,Tvs(1))
%% --------------------------------- Open raceway pond --------------------------------------------------------
THY=8760;       %Total hours per year (hr/yr)
CO2_emission_ZLD=5.365892783;
CO2_emission_per_distillate_ZLD=CO2_emission_ZLD/Dtotal_ZLD
[Pond_area,NS_growth,Cost_of_land,Cost_of_pond]=Open_raceway_pond(CO2_emission,THY)

%% --------------------------------- Cost of PV --------------------------------------------------------
Total_electrical=1072.382606
[Cost]=Cost_PV(Total_electrical);
Capital_PV=Cost(1)
Operating_PV=Cost(2)
% %% --------------------------------- Evaporation pond --------------------------------------------------------
...
Capex_equipment=Cintake+Cmed+Ccondenser+Ctvc;
Capex_civilwork=0.15*Capex_equipment;
Capex_direct=Capex_equipment+Capex_civilwork
Capex_indirect=0.25*Capex_direct
Capex_ZLD=33341800;
Tcc=Capex_direct+Capex_indirect+Capex_ZLD+Cost_of_land+Cost_of_pond+collector_CC+TES_cost+BHX_CC        %Total capital cost ($)
% ------------------------ Csteam calculation -----------------------------
if (Pms<=101.325)
    Csteam_TVC=Csteam;
else 
...
AOC_ZLD=14460700;
AOC=AOCchem+AOClab+AOCpow+AOCman+AOCsteam_TVC+AOC_ZLD+collector_OC+BHX_OC
%----------------------------------- Total Annual cost--------------------
...
FWC=TAC*(Dtotal_ZLD*THY*3600)^(-1);          %Freshwater cost in $/kg
...
% N=(Brine*THY*3600)/(Xs(n)*58.44+(1-Xs(n))*18.01528);    %kmoles
% R=8.31447;    % kJ/kmolK
% T=Tbs(n)+273.15; %K
% moles_of_salt=((Brine*THY*3600)*Xs(n))/58.44;
% moles_of_water=((Brine*THY*3600)*(1-Xs(n)))/18.01528;
...
Wmin=Devaporator*Exergy_distillate_out+Mcw*Exergy_coolseawater_out+Brine*Exergy_brine_out;
...
Out=[Efficiency Devaporator CO2_emission tac];
else
