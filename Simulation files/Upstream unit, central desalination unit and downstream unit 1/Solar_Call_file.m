...
Electrical=(Ftotal*(2*10^5))/(rhol(Tf,10^6*Xf))+(Brine*((101325-Psaturation(Tbs(n))*10^3)))/(rhol(Tbs(n),10^6*Xs(n)))+...
    (Devaporator*((101325-Psaturation(Tcon)*10^3)))/(rhol(Tcon,10^6*0)); % Assuming the discharge pressure of the feed pump is 2 bar
Electrical_kW=Electrical/1000;
Eevaporator=Electrical_kW
E_pump1_aspen=0.16805061100000002
E_pump2_aspen=7.7146833500000005
Epump_feed=(Ftotal*(2*10^5))/(rhol(Tf,10^6*Xf))/1000
Epump_distillate=(Devaporator*((101325-Psaturation(Tcon)*10^3)))/(rhol(Tcon,10^6*0))
Epump_brine=(Devaporator*((101325-Psaturation(Tcon)*10^3)))/(rhol(Tcon,10^6*0))
...
% Qfuel_turbine=0;
Water_utility=12582.377668; %kg/s from Aspen
...
CO2_emission_solar=5.040403056+CO2_emission_water
...
Cap_aspen=42127500;
Capex_equipment=Cintake+Cmed+Ccondenser+Ctvc+collector_CC+TES_cost+BHX_CC;
...
% AOC_ASPEN=458597000;
AOC_ASPEN=16810900;
AOC=AOCchem+AOClab+AOCpow+AOCman+AOCsteam_TVC+collector_OC+BHX_OC+AOC_ASPEN
AOC_m3=(AOC*life)/((Dtotal/1000)*3600*THY*life)
%----------------------------------- Total Annual cost--------------------
Salt=46.3988593287717
Capital_recovery_factor=(Ir*((1+Ir)^life))/(((1+Ir)^life)-1)   %$(1/yr)
Salt_SP=(46.3988593287717*3600*THY)*0.3
% FWC_salt=Salt_SP/(46.3988593287717*THY*3600);
% tac_salt=FWC_salt*2160
TAC=AOC+Tcc*Capital_recovery_factor+Total_cost_ORP+cost_lipid_extraction_per_year+TAC_PV-Salt_SP-SP_lipids        %Total annual cost ($/yr)
...
Tdistillate=((Devaporator*(Tcw+273)+Daspen*360.194340092048)/Dtotal)-273;
