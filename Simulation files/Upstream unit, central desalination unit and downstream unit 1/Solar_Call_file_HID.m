...
Msw=Mcw+Ftotal
Daspen=706.479106659132
Dtotal=Devaporator+Daspen
Recovery=Dtotal/Msw
Salt=34.1259303019058 % kg/s
Salt_kg_hr=Salt*3600;
Salt_kg_day=Salt*3600*24;
salt_density=2165;
salt_m3_day=Salt_kg_day/salt_density
%% -------------------------------------- CO2 emission -------------------------------------------------------------
...
% Hwater=hb(Tv(1));
% Q_flash_aspen=11416.8406;
Q_flash_aspen=0; % kJ/s
Q_crystallizer_aspen=539462.191; % kJ/s
Qtotal=Qproc+Q_flash_aspen+Q_crystallizer_aspen
Q=Qtotal+Mms*4.187*(100-Tvs(1))+Mms*Lamda(100)+Mms*1.9*(Tsaturation(Pms)-100)   % Extra heat for converting the condensate from Tv(1) to steam temperature
% Qfuel_evap=((Qproc/Lamda_proc_TVC)*(Hproc_TVC-Hwater_TVC))*((Tftb-Ta)/(Tftb-Tstack))
Qfuel_evap=((Q/Lamda_proc_TVC)*(Hproc_TVC-Hwater_TVC))*((Tftb-Ta)/(Tftb-Tstack)) % Extra fuel is also consumed for the excess heat for converting condensate to steam temperature
Mms_total=Qtotal/(Lamda_proc_TVC)
...
Electrical=(Ftotal*(2*10^5))/(rhol(Tf,10^6*Xf))+(Brine*((101325-Psaturation(Tbs(n))*10^3)))/(rhol(Tbs(n),10^6*Xs(n)))+...
    (Devaporator*((101325-Psaturation(Tcon)*10^3)))/(1000); % Assuming the discharge pressure of the feed pump is 2 bar
Electrical_kW=Electrical/1000;
Eevaporator=Electrical_kW
E_pump1_aspen=17.0099898
E_pump2_aspen=29.156569500000003
Epump_feed=((Ftotal*(2*10^5))/(rhol(Tf,10^6*Xf)))/1000
Epump_distillate=((Devaporator*((101325-Psaturation(Tcon)*10^3)))/(1000))/1000
Epump_brine=(Brine*((101325-Psaturation(Tbs(n))*10^3)))/(rhol(Tbs(n),10^6*Xs(n)))/1000
...
% Qfuel_turbine=0;
Water_utility=9074.25+4801.33 %kg/s from Aspen
CO2_emission_water=(Water_utility/1000)*0.59
CO2_emission=((Qfuel_evap+Qfuel_turbine)/NHV)*(C_percent/100)*alpha %kg/sec
% CO2_emission_per_distillate=CO2_emission/Dtotal
CO2_emission_total_nonsolar=CO2_emission+CO2_emission_water
%% --------------------------------- Solar calculation --------------------------------------------------------
Qprocess=Q; %kJ/s
[collector_CC, collector_OC,TES_cost, BHX_CC,BHX_OC,Asolar,q_ra0] = PTSC_Solar_TVC(Qprocess,Mms,Pms,Tvs(1),Water_utility)
%% --------------------------------- Open raceway pond --------------------------------------------------------
THY=8760;       %Total hours per year (hr/yr)
CO2_emission_solar=0.558969316+CO2_emission_water
...
AHEX1=3846.63206802935
C_HEX1=Kcond*Cmat_Cond*(AHEX1^0.8)
AHEX2=35688.4436976832
C_HEX2=Kcond*Cmat_Cond*(AHEX2^0.8)
AHEX3=1104.01349197755
C_HEX3=Kcond*Cmat_Cond*(AHEX3^0.8)
AHEX4=47701.0342847735
C_HEX4=Kcond*Cmat_Cond*(AHEX4^0.8)
Cintake=(Kintake*24*3600*Msw)/rhol(Tf,10^6*Xf);
Vflash=993.065829019742;
Cflash=6500*(Vflash^0.62)
Ccrystallizer=28531*log(salt_m3_day)+18824
Cfilter=1000*Salt_kg_hr %(https://www.mdpi.com/2297-8739/9/10/295#B38-separations-09-00295)
Cpump_feed=(12434*(Epump_feed*1.34)^0.5)
Cpump_brine=(12434*(Epump_brine*1.34)^0.5)
Cpump_distillate=(12434*(Epump_distillate*1.34)^0.5)
Cpump_pump1=(12434*(E_pump1_aspen*1.34)^0.5)
Cpump_pump2=(12434*(E_pump2_aspen*1.34)^0.5)
Cap_aspen=0;
Capex_equipment=Cintake+Cmed+Ccondenser+Ctvc+collector_CC+TES_cost+BHX_CC+C_HEX1+C_HEX2+C_HEX3+C_HEX4+Cflash+Ccrystallizer+Cfilter+...
Cpump_feed+Cpump_brine+Cpump_distillate+Cpump_pump1+Cpump_pump2;
...
Tcc_m3=(Tcc*life)/((Dtotal/1000)*3600*THY*life)
% ------------------------ Csteam calculation -----------------------------
if (Pms<=101.325)
    Csteam_TVC=Csteam;
else 
...
AOClab=(Clab*THY*3600*Dtotal)/rho_d                   %Cost of human labour
...
AOC_pumps=Cpow*THY*Etotal
AOCpow=AOC_pumps
AOCchem=(Cchem*THY*3600*Dtotal)/rho_d         %Annual operating cost of chemicals
AOC_utility=9.46979e+06+5.01061e+06                          % Cooling water utility cost $/year from ASPEN 
AOC_ASPEN=0;
AOC=AOCchem+AOClab+AOCpow+AOCman+AOCsteam_TVC+collector_OC+BHX_OC+AOC_ASPEN+AOC_utility
%----------------------------------- Total Annual cost--------------------
SP_salt=(Salt_kg_hr*THY)*0.3
Capital_recovery_factor=(Ir*((1+Ir)^life))/(((1+Ir)^life)-1)   %$(1/yr)
TAC=AOC+Tcc*Capital_recovery_factor+Total_cost_ORP+cost_lipid_extraction_per_year+TAC_PV        %Total annual cost ($/yr)
TAC_profit=SP_lipids+SP_salt-TAC
...
FWC_profit=TAC_profit*(Dtotal*THY*3600)^(-1);
tac=FWC*1000                               %Freshwater cost in $/m^3
tac_profit=FWC_profit*1000
% tac_salt=FWC_salt*2160
%% ------------------------------------------- Exergy efficiency ----------------------------------------------
...
Tdistillate=((Devaporator*(Tcw+273)+Daspen*(326))/Dtotal)-273;
...
Exergy_utility_out=exergy_calculation(101.325,60,0,Tcw,Xf);
Exergy_utility_in=exergy_calculation(101.325,Tcw,0,Tcw,Xf);
Exergy_distillate_out=exergy_calculation(101.325,Tdistillate,0,Tcw,Xf);
Tambient=Tcw+273;
Tsun=5770;  % Temperature of sun (K)
Exergy_solar_in=Q*(1-(4/3)*(Tambient/Tsun)+(1/3)*((Tambient/Tsun)^4)); %kW
ST=q_ra0;
Apv=Area_PV;
Exergy_PV_in=(1-Tambient/Tsun)*(ST*(10^(-3)))*Apv; %kW
% Tsteam=Tsaturation(Pms)+273.15;
% % Thcrystallizer=Ts+273.15;
% Q=(Mms*Lamda(Tsaturation(Pms)))*(1-Tout/Tsteam);
...
Wmin=Dtotal*(Exergy_distillate_out-0)+Wsep;
Efficiency=(Wmin/(Exergy_solar_in+Exergy_PV_in))*100;

%% ----------------------------------------------------------------------------------------
% A=[Efficiency Devaporator CO2_emission tac];
A=[Efficiency Dtotal CO2_emission_solar tac]; %Solar
k=length(A);
E=isnan(A);
R=any(E);
